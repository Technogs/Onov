...
class FeedDetailActivity : BaseAppCompatActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        feedData = intent.getParcelableExtra<FeedsData>("feed")
        eventData = intent.getParcelableExtra<EventData>("event")
        init()
        ...
        if (feedData != null) {
            ...
            if (feedData?.recordType == "petition") {
                ...
                binding.uprRlyt.setOnClickListener {
                    val intent =
                        Intent(this@FeedDetailActivity, PetitionDetailsActivity::class.java)
            ...
            when (feedData?.fileType) {
                ...
                "photo" -> {
                    ...
                    if (feedData?.recordType == "petition") {
                        ...
                        Glide.with(this).load(BaseUrl.photoUrl + feedData?.pollImage)
                            .into(binding.ivFeed)
                    } else Glide.with(this).load(BaseUrl.photoUrl + feedData?.filePath)
                        .into(binding.ivFeed)
            ...
            if (successful != null) {
                if (successful) {
                    if (homeViewModel.status == "success") {
                        if (feedData?.Liked == "1") {
                            binding.iconLike.setColorFilter(resources.getColor(R.color.red))
                        } else if (feedData?.Disliked == "1") {
                            binding.iconDislike.setColorFilter(resources.getColor(R.color.red))
                        }
                        ...
                        if (homeViewModel.message == "Disliked successfully") {
                            ...
                            if ((binding.tvLikeCount.text).toString().toInt() > 0)
                                binding.tvLikeCount.text =
                                    ((binding.tvLikeCount.text).toString().toInt() - 1).toString()
                            binding.tvDislikeCount.text =
                                ((feedData?.dislikeCount)!!.toInt() + 1).toString()
                        } else if (homeViewModel.message == "Liked successfully") {
                            likeDataClass?.like = true
                            binding.iconLike.setColorFilter(resources.getColor(R.color.red))
                            binding.iconDislike.colorFilter = null
                            binding.tvLikeCount.text =
                                ((feedData?.likeCount)!!.toInt() + 1).toString()
                            if ((binding.tvDislikeCount.text).toString().toInt() > 0)
                                binding.tvDislikeCount.text =
                                    ((binding.tvDislikeCount.text).toString()
                                        .toInt() - 1).toString()
