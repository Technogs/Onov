...
class SearchFriendsAdapter(
    val context: Context,
    var list: List<Follow>,
    val onviewClick: OnViewClick
) : RecyclerView.Adapter<SearchFriendsAdapter.RVHolder>() {


    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RVHolder {
        val binding =
            RvSearchFriendsBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    override fun onBindViewHolder(holder: RVHolder, position: Int) {
        ...
        holder.binding.ivSearchProfile.setOnClickListener {
            ...
            intent.putExtra("usrRef", list[position].userRef2)
    ...
    inner class RVHolder(val binding: RvSearchFriendsBinding) :
        ...
        fun bind(searchData: Follow) {
            binding.chatUserName.text = searchData.fullName
            Glide.with(context).load(BaseUrl.photoUrl + searchData.profilePic)
                .apply(RequestOptions().placeholder(R.drawable.ic_baseline_account_circle_24))
                .into(binding.ivProfilepic)

            if (searchData.follow == "0") {
                binding.follow.text = "Support"
            } else {
                binding.follow.text = "UnSupport"
            }
