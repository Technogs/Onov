package com.application.onovapplication.debate.vlive.ui.live;
...
import com.application.onovapplication.debate.vlive.protocol.model.request.AudienceListRequest;
import com.application.onovapplication.debate.vlive.protocol.model.request.ModifySeatStateRequest;
import com.application.onovapplication.debate.vlive.protocol.model.request.ModifyUserStateRequest;
import com.application.onovapplication.debate.vlive.protocol.model.request.Request;
import com.application.onovapplication.debate.vlive.protocol.model.request.SeatInteractionRequest;
...
import com.application.onovapplication.debate.vlive.protocol.model.response.Response;
import com.application.onovapplication.debate.vlive.protocol.model.response.SeatStateResponse;
import com.application.onovapplication.debate.vlive.protocol.model.types.SeatInteraction;
import com.application.onovapplication.debate.vlive.ui.actionsheets.InviteUserActionSheet;
import com.application.onovapplication.debate.vlive.ui.actionsheets.toolactionsheet.LiveRoomToolActionSheet;
import com.application.onovapplication.debate.vlive.ui.components.CameraSurfaceView;
import com.application.onovapplication.debate.vlive.ui.components.CameraTextureView;
import com.application.onovapplication.debate.vlive.ui.components.LiveMessageEditLayout;
import com.application.onovapplication.debate.vlive.ui.components.LiveMultiHostSeatLayout;
import com.application.onovapplication.debate.vlive.ui.components.SeatItemDialog;
import com.application.onovapplication.debate.vlive.ui.components.VoiceIndicateGifView;
...
public class MultiHostLiveActivity extends LiveRoomActivity implements View.OnClickListener,DebateJoineeAdapter.onVoteClick,
...
//    SearchViewModel searchViewModel = new ViewModelProvider(MultiHostLiveActivity.this).get(SearchViewModel.class);
   BottomSheetDialog bottomSheetDialog;
    List<JoinerData> joinerData;
    @Override
    public void voteBtnClick(@NotNull JoinerData joinerData) {
        voteDebator(joinerData.getUserRef());
        bottomSheetDialog.dismiss();
    }


    private void showBottomSheetDialog(List<JoinerData> joinerData) {

         bottomSheetDialog = new BottomSheetDialog(MultiHostLiveActivity.this);
        bottomSheetDialog.setContentView(R.layout.view_people_going);
        DebateJoineeAdapter followersAdapter;

        RecyclerView recyclerView = bottomSheetDialog.findViewById(R.id.rv_people_going);
        AppCompatImageView share = bottomSheetDialog.findViewById(R.id.ivCancel);
    followersAdapter = new DebateJoineeAdapter(MultiHostLiveActivity.this,joinerData,this,"");
...
share.setOnClickListener(new View.OnClickListener() {
    ...
    private void voteDebator(String voteTo) {
        //alertDialog.show();
        ...
        call.enqueue(new Callback<DebateJoinerResponse>() {
            ...
            @Override
            public void onResponse(Call<DebateJoinerResponse> call, retrofit2.Response<DebateJoinerResponse> response) {
                DebateJoinerResponse response1=response.body();

                if (response1.getStatus().equalsIgnoreCase("success")){
 joinerData=response1.getJoinerData();
...
// showBottomSheetDialog(joinerData);
            ...
            @Override
            public void onFailure(Call<DebateJoinerResponse> call, Throwable t) {
//                alertDialog.dismiss();
                if (t instanceof IOException) {
                    Toast.makeText(MultiHostLiveActivity.this, "Something went wrong, Please try again", Toast.LENGTH_SHORT).show();
...
//        BottomSheetDialog mBottomSheetDialog= new BottomSheetDialog(MultiHostLiveActivity.this, R.style.SheetDialog);
...
//
//           View sheetView = layoutInflater.inflate(R.layout.view_people_going, null);
//            mBottomSheetDialog.setContentView(sheetView);
//            mBottomSheetDialog.show();
//            RecyclerView recyclerView=sheetView.findViewById<RecyclerView>(R.id.rv_people_going)
//                    AppCompatImageView         imageView=sheetView.findViewById<AppCompatImageView>(R.id.ivCancel)
//
////changed
//                    //  sheetView.let {
...
//
//
//            imageView.setOnClickListener { mBottomSheetDialog.dismiss() }
//        }
    ...
    private void initUI() {
        ...
        AppCompatTextView   joineeButton = findViewById(R.id.votedebater);
        bottomButtons.init();
        ...
        findViewById(R.id.votedebater).setOnClickListener(this);
        ...
        if (isOwner) {
             ...
             mOwnerUIManager.setOwnerName(userPreferences.getuserDetails().getFullName());
//             mOwnerUIManager.setOwnerName(config().getUserProfile().getUserName());
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.live_bottom_btn_fun2:
                ...
                break;
                case R.id.votedebater:
           //    updateTasks();
                    Log.d("joineedata",""+userPreferences.getDebateJoiner().getJoinerData());
                    showBottomSheetDialog(userPreferences.getDebateJoiner().getJoinerData());
    ...
    public void onEnterRoomResponse(EnterRoomResponse response) {
        ...
        if (response.code == Response.SUCCESS) {
          ...
          //  config().getUserProfile().setUserId(userPreferences.getuserDetails().getId());
            ...
            runOnUiThread(() -> {
                response.data.room.owner.userName=userPreferences.getuserDetails().getFullName();
                mOwnerUIManager.setOwnerName(userPreferences.getuserDetails().getFullName());
